...
impl CompressStream {
    ...
    fn and_buffer<'a>(
        ...
        // Create an `ArrayBuffer` backed by a `Vec<u8>` containing the written data
        Ok(JsArrayBuffer::external(cx, output))
    }
}
...
fn compress_chunk(mut cx: FunctionContext) -> JsResult<JsPromise> {
    ...
    // Skip the 2nd argument (encoding) and grab the 3rd argument as a `Uint8Array`. The
    // data is immediately converted to a `Vec<u8>` by borrowing as a `&[u8]` and cloning.
